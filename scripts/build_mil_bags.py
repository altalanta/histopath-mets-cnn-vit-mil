...
def create_mil_bags(
    ...
    for slide_id, slide_tiles in tqdm(
        tiles_df.groupby("slide_id"), desc="Creating bags"
    ):
        ...
        if n_tiles < min_tiles_per_bag:
            slides_filtered += 1
            logging.debug(
                f"Skipping slide {slide_id}: only {n_tiles} tiles (min: {min_tiles_per_bag})"
            )
            continue
...
def add_metadata_to_bags(bags_df: pd.DataFrame, metadata_path: Path) -> pd.DataFrame:
    ...
    bags_with_meta = bags_df.merge(
        slide_metadata[["slide_id", "split", "patient_id"]], on="slide_id", how="left"
    )
...
def save_bags_data(
    bags_df: pd.DataFrame, output_dir: Path, format: str = "parquet"
) -> None:
    ...
    if format == "parquet":
        ...
        for col in ["tile_ids", "coords", "tile_paths"]:
            if col in bags_save.columns:
                bags_save[col] = bags_save[col].apply(
...
def generate_bags_summary(bags_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    if "split" in bags_df.columns:
        split_summary = (
            bags_df.groupby(["split", "label"])
            .agg({"bag_id": "count", "n_tiles": ["sum", "mean"]})
            .round(1)
        )
        ...
        for (split, label), row in split_summary.iterrows():
            ...
            report_lines.append(
                f"| {split} | {label_name} | {n_bags} | {total_tiles:,} | {avg_tiles:.1f} |"
            )
    ...
    for label, count in label_dist.items():
        label_name = (
            "Normal" if label == 0 else "Tumor" if label == 1 else f"Label_{label}"
        )
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Build MIL bags from tile data")

    parser.add_argument(
        "--tiles", type=Path, required=True, help="Directory containing tiles index"
    )

    parser.add_argument(
        "--out", type=Path, required=True, help="Output directory for bags data"
    )

    parser.add_argument(
        "--min-tiles", type=int, default=50, help="Minimum tiles per bag (default: 50)"
    )
    ...
    logging.info(
        f"Loaded {len(tiles_df)} tiles from {tiles_df['slide_id'].nunique()} slides"
    )
    ...
    if "split" in bags_df.columns:
        for split in bags_df["split"].unique():
            if pd.notna(split):
                ...
                split_simple = split_bags[
                    ["bag_id", "slide_id", "label", "n_tiles"]
                ].copy()
