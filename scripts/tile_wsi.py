...
from histopath.utils.tissue_mask import (
    create_tissue_mask,
    filter_tile_by_tissue_content,
)
...
def create_tissue_mask_for_slide(
    slide_path: Path, downsample: int = 32
) -> Optional[np.ndarray]:
    ...
    try:
        ...
        tissue_mask = create_tissue_mask(
            thumbnail_array, method="otsu", min_tissue_ratio=0.1
        )
    ...
    try:
        ...
        if tissue_filter:
            ...
            if tissue_mask is None:
                logging.warning(
                    f"Failed to create tissue mask for {slide_id}, skipping tissue filtering"
                )
        ...
        for i, (x, y, level) in enumerate(coordinates):
            try:
                ...
                if tissue_filter:
                    ...
                    if not filter_tile_by_tissue_content(
                        tile_array, tissue_threshold=0.3
                    ):
                        continue
                ...
                tile_data = {
                    ...
                    "label": label,
                    "tile_path": (
                        str(tile_path.relative_to(output_dir)) if tile_path else None
                    ),
        ...
        logging.info(
            f"Successfully processed {len(tiles_data)} tiles for slide {slide_id}"
        )
        return tiles_data
...
def save_tiles_index(
    tiles_df: pd.DataFrame, output_dir: Path, format: str = "parquet"
) -> None:
    ...
    if "slide_id" in tiles_df.columns:
        ...
        if metadata_path.exists():
            ...
            tiles_with_split = tiles_df.merge(
                slide_metadata[["slide_id", "split"]], on="slide_id", how="left"
            )

            split_summary = (
                tiles_with_split.groupby(["split", "label"])
                .size()
            ...
            for split in split_summary.index:
                normal = (
                    split_summary.loc[split, 0] if 0 in split_summary.columns else 0
                )
...
def main():
    ...
    parser.add_argument(
        "--out", type=Path, required=True, help="Output directory for tiles"
    )

    parser.add_argument(
        "--tile-size", type=int, default=256, help="Tile size in pixels (default: 256)"
    )
