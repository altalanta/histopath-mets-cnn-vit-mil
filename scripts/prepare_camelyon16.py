...
def create_slide_metadata(raw_data_dir: Path) -> pd.DataFrame:
    ...
    for slide_type in ["normal", "tumor"]:
        ...
        for slide_path in slide_dir.glob("*.tif"):
            ...
            if slide_type == "tumor":
                mask_path = (
                    raw_data_dir / "training" / "masks" / f"{slide_path.stem}_mask.tif"
                )
                slide_info["mask_path"] = (
                    str(mask_path.relative_to(raw_data_dir))
                    if mask_path.exists()
                    else None
                )
            else:
...
def create_patient_splits(
    ...
    val_mask = (metadata_df["split"] == "train") & (
        metadata_df["patient_id"].isin(val_patient_ids)
    )
...
def validate_data_integrity(metadata_df: pd.DataFrame, raw_data_dir: Path) -> bool:
    ...
    if len(train_labels) > 0:
        ...
        if imbalance_ratio > 5:
            issues.append(
                f"High class imbalance in training set: ratio = {imbalance_ratio:.2f}"
            )
        ...
        if len(split_data) > 0:
            ...
            for label, count in label_dist.items():
                if label >= 0:  # Skip unknown labels (-1)
                    ...
                    report_lines.append(
                        f"| {label_name} ({label}) | {count} | {pct:.1f}% |"
                    )
...
def main():
    ...
    parser.add_argument(
        "--out", type=Path, required=True, help="Output directory for processed data"
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for splits (default: 42)"
    )
