...
class HistopathMetrics:
        ...
        if bootstrap_ci_metrics and y_proba is not None:
            ...
            for metric_name in metrics_for_ci:
                if metric_name in ["auroc", "auprc"]:
                    y_proba_ci = (
                        y_proba[:, 1]
                        if self.num_classes == 2 and y_proba.ndim > 1
                        else y_proba
                    )
    ...
    def plot_roc_curve(
        self, y_true: np.ndarray, y_proba: np.ndarray, figsize: Tuple[int, int] = (8, 6)
    ) -> plt.Figure:
        """
        Plot ROC curve.
    ...
    def plot_calibration_curve(
        ...
        ax.plot(
            [0, 1], [0, 1], "k--", linewidth=1, alpha=0.7, label="Perfect Calibration"
        )
...
class MILMetrics(HistopathMetrics):
    ...
    def _gini_coefficient(self, x: np.ndarray) -> float:
        ...
        gini = (2 * np.sum((np.arange(1, n + 1) * sorted_x))) / (n * cumsum[-1]) - (
            n + 1
        ) / n
    ...
    def plot_attention_distribution(
        ...
        box_data = [
            class_weights[i] for i in range(self.num_classes) if class_weights[i]
        ]
        box_labels = [
            self.class_names[i] for i in range(self.num_classes) if class_weights[i]
        ]
        ...
        entropy_data = [
            entropies_by_class[i]
            for i in range(self.num_classes)
            if entropies_by_class[i]
        ]
