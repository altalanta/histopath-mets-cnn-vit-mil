        ...
        if trainer.current_epoch == self.unfreeze_epoch and not self.unfrozen:
            if hasattr(pl_module, "unfreeze_backbone"):
                ...
                trainer.logger.log_metrics(
                    {"backbone_unfrozen": 1}, step=trainer.global_step
                )
...
class GradientLoggingCallback(pl.Callback):
    ...
    def on_after_backward(
        ...
        if trainer.global_step % self.log_frequency == 0:
            ...
            # Log gradient norm
            trainer.logger.log_metrics(
                {"grad_norm": total_norm, "grad_norm_log": np.log10(total_norm + 1e-8)},
...
class AttentionLoggingCallback(pl.Callback):
    ...
    def _log_attention_weights(
        self, trainer: pl.Trainer, pl_module: pl.LightningModule, batch: Dict[str, Any]
    ) -> None:
...
class ModelStatisticsCallback(pl.Callback):
    ...
    def _log_model_stats(
        ...
        important_params = [
            k
            for k in param_stats.keys()
            if any(layer in k for layer in ["classifier", "attention", "fc", "head"])
        ]

        filtered_stats = {
            k: param_stats[k] for k in important_params[:10]
        }  # Log top 10
