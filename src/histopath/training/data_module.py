...
class HistopathDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.train_transform = HistopathTransforms.get_train_transforms(
            size=image_size, normalize=normalize, augment_prob=augment_prob
        )
        self.val_transform = HistopathTransforms.get_val_transforms(
            size=image_size, normalize=normalize
        )
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not tiles_path.exists():
            raise FileNotFoundError(f"Tiles index file not found: {tiles_path}")
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load tiles index from {tiles_path}: {e}"
            ) from e

        # Validate tiles dataframe
        if tiles_df.empty:
            raise ValueError("Tiles index is empty")
        required_columns = ["tile_path", "label"]
        missing_columns = [
            col for col in required_columns if col not in tiles_df.columns
        ]
        if missing_columns:
            raise ValueError(
                f"Missing required columns in tiles index: {missing_columns}"
            )
        ...
        if metadata_path.exists():
            try:
                ...
                tiles_df = tiles_df.merge(
                    metadata_df[["slide_id", "split"]], on="slide_id", how="left"
                )
            except Exception as e:
                print(f"Warning: Failed to load slide metadata: {e}")
        ...
        if stage == "fit" or stage is None:
            ...
            self.val_dataset = HistopathDataset(
                tiles_df=val_tiles, transform=self.val_transform, root_dir=self.data_dir
            )
