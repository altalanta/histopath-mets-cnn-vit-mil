...
class BaseHistopathModule(pl.LightningModule):
    """Base class for histopathology Lightning modules."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initialize base module with common parameters."""
        super().__init__()
    ...
    def _log_metrics(self, stage: str, loss: torch.Tensor, preds: torch.Tensor, 
                    labels: torch.Tensor, probs: torch.Tensor, prog_bar: bool = False) -> None:
        ...
        self.log(f'{stage}/loss', loss, on_step=(stage=='train'), on_epoch=True, prog_bar=prog_bar)
        self.log(f'{stage}/acc', accuracy_metric, on_step=(stage=='train'), on_epoch=True, prog_bar=prog_bar)
        
        # Log additional metrics for val/test
        if stage in ['val', 'test']:
            ...
            if self.hparams.num_classes == 2:
                auroc_metric(probs[:, 1], labels)
            else:
                auroc_metric(probs, labels)
            
            self.log(f'{stage}/auroc', auroc_metric, on_step=False, on_epoch=True, prog_bar=prog_bar)
            self.log(f'{stage}/precision', precision_metric, on_step=False, on_epoch=True)
            self.log(f'{stage}/recall', recall_metric, on_step=False, on_epoch=True)
            self.log(f'{stage}/f1', f1_metric, on_step=False, on_epoch=True)


class HistopathClassifier(BaseHistopathModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: Additional arguments for BaseHistopathModule
        """
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        self._log_metrics('train', loss, preds, labels, probs, prog_bar=True)
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        preds = torch.argmax(logits, dim=1)
        probs = F.softmax(logits, dim=1)
        self._log_metrics('val', loss, preds, labels, probs, prog_bar=True)
    ...
    def test_step(self, batch: Dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        preds = torch.argmax(logits, dim=1)
        probs = F.softmax(logits, dim=1)
        self._log_metrics('test', loss, preds, labels, probs)
...
class MILClassifier(BaseHistopathModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Additional storage for MIL-specific data
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        # Forward pass
        logits, _ = self.forward(images, return_attention=True)
        loss = self.criterion(logits, labels)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if hasattr(self.logger, 'experiment') and hasattr(self.logger.experiment, 'log_confusion_matrix'):
        ...
        if hasattr(self.logger, 'save_dir') and self.logger.save_dir:
