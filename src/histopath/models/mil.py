...
class TransMIL(nn.Module):
    ...
    def forward(
        ...
        # Average attention weights across layers if available
        if attention_weights and len(attention_weights) > 0:
            # Attention to CLS token (first position)
            cls_attention = torch.stack([
                att[:, :, 0, 1:].mean(dim=1)  # Average across heads, get CLS->patch attention
                for att in attention_weights
            ]).mean(dim=0)  # Average across layers
            attention_weights = cls_attention.unsqueeze(-1)  # (B, N, 1)
